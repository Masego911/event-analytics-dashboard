...
public class TicketBuyerWebApp {
    ...
    private static final File DATA_DIR = new File("app_data");
    private static final File GENERATED_DIR = new File(DATA_DIR, "generated");

    private static File dataDirectory() {
        if (!DATA_DIR.isDirectory()) throw new IllegalStateException("Missing data directory: " + DATA_DIR.getAbsolutePath());
        return DATA_DIR;
    }
    ...
    private static void handleShowEmails(HttpExchange ex) throws IOException {
        ProcessedData data=processFolder(dataDirectory());
    ...
    private static void handleCategoryContacts(HttpExchange ex) throws IOException {
        File folder=dataDirectory(); ProcessedData data=processFolder(folder);
    ...
    private static void handleLogo(HttpExchange ex) throws IOException {
        File folder=dataDirectory(); File use=null; String mime="image/png";
    ...
    private static void handleUpload(HttpExchange ex) throws IOException {
        ...
        ByteArrayOutputStream baos=new ByteArrayOutputStream();try(InputStream in=ex.getRequestBody()){byte[]buf=new byte[4096];int r;while((r=in.read(buf))!=-1){if(baos.size()+r>maxUploadBytes){sendHtml(ex,413,"<html><body><p>Upload is too large (25 MB maximum).</p><a href='/'>Back</a></body></html>");return;}baos.write(buf,0,r);}}
        ...
        for(String part:body.split("--"+java.util.regex.Pattern.quote(boundary))){
            ...
            fileName=fileName.replace('\\','/');int slash=fileName.lastIndexOf('/');if(slash>=0)fileName=fileName.substring(slash+1);fileName=sanitize(fileName);
            if(fileName.isEmpty()||fileName.equals(".csv"))fileName="uploaded_"+System.currentTimeMillis()+".csv";
            if(!fileName.toLowerCase().endsWith(".csv"))fileName+=".csv";
            try(FileOutputStream fos=new FileOutputStream(new File(folder,fileName))){fos.write(dBlock.getBytes(StandardCharsets.ISO_8859_1));}
            invalidateDataCache();
            saved=true;
            break;
        }
        if(saved){ex.getResponseHeaders().set("Location","/?upload=success");ex.sendResponseHeaders(303,-1);ex.close();}
        else sendHtml(ex,400,"<html><body><p>No CSV file was found in the upload.</p><a href='/'>Back</a></body></html>");
    }
    ...
    private static ProcessedData loadFolder(File folder) {
        ...
        if (!GENERATED_DIR.exists() && !GENERATED_DIR.mkdirs()) throw new IllegalStateException("Cannot create generated directory");
        writeContactsWithShows(new File(GENERATED_DIR,"contacts_with_shows.csv"),data.contactEmails,data.contactPhones,data.contactShows);
        return data;
    }
